...
点击“更多颜色…”后，关闭主题色板并在同一锚点打开二级非模态颜色面板。普通用户无需看到 RGB 字段，只有需要精确颜色时才进入该层。

### 面板内容

- 面板约 `244 × 225px`，不改变工具栏或卡片布局。
- 左侧为色相盘，用于快速探索颜色。
- 右侧显示颜色预览。
...
- 色相盘变化后，同步 RGB、HEX 和预览色。
- RGB 输入变化后，同步色相盘位置、HEX 和预览色。
- RGB 仅接受整数 `0–255`。
...
生产实现优先采用成熟的 React 色相盘组件，避免手写指针坐标、HSL/RGB 转换和键盘行为。引入前应检查包体积、维护状态和许可证；适配层只向编辑器暴露标准 HEX 字符串。

若候选依赖不满足要求，则使用原生颜色转换工具和受测的纯函数实现，色相盘交互仍须具备键盘替代路径，RGB 输入始终是精确操作入口。

## 选区与编辑器行为

所有字号和颜色操作复用当前选区保护机制：

1. 工具栏 `mousedown` 捕获阶段保存 `editor.prosemirrorState.selection`。
2. 阻止工具栏控件抢走编辑器焦点。
3. 命令执行前恢复保存的 Selection。
4. 字号通过 `fontSize` mark 写入，例如 `14px`。
5. 颜色通过 BlockNote `editor.addStyles({ textColor: "#0891B2" })` 写入。
6. 默认颜色操作使用 `removeStyles`，而不是写入伪造的默认色。
